...
class DomainRecord(object):
    ...
    def create_new(cls, domain_name, data, authentication_token):
        ...
        create_domain_record_response = utils.post_request("domains/{0}/records"
                .format(domain_name), data, authentication_token)

        if not create_domain_record_response.ok:

            raise Exception("Failed to create domain record! See error message: 
                    {0}".format(create_domain_record_response.json()["message"]))
    ...
    def from_existing(cls, domain_name, record_id, authentication_token):
        ...
        get_domain_record_response = utils.get_request("domains/{0}/records/{1}"
                .format(domain_name, record_id), authentication_token)

        if not get_domain_record_response.ok:

            raise Exception("Failed to get domain record! See error message: {0}"
                    .format(get_domain_record_response.json()["message"]))
    ...
    def delete(self):
        ...
        if not delete_domain_record_response.ok:
            ...
            raise Exception("Failed to delete domain record! See error message: {0}"
                    .format(delete_domain_record_response.json()["message"]))
